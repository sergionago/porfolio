...
function añoAjaxAn() {
  ...
  xhr.open("GET", "../back/php.php?barrasAnual="+"yes", true);
  xhr.send();
}
...
function añoAjaxMes() {
  ...
  xhr.open("GET", "../back/php.php?yearMes=" + yearMes, true);
  xhr.send();
}
...
function añoAjaxSem() {
  ...
  xhr.open("GET", "../back/php.php?yearSem=" + yearSem + "&mesSem=" + mesSem, true);
  xhr.send();
}
...
function nextBoton() { //Botón calendario siguiente
  ...
  if (tiempo_seleccionado === "Mensual") {
    if (parseInt(span.innerText) < 2023) {
      añoActual = parseInt(span.innerText);
...
function obtenerDatosYCrearTabla(){
  ...
  xhr.open("GET", "../back/php.php?accion=datos_tabla", true);
  xhr.send();
}
...
function borrarReg(id_del) {
  let url = `../back/php.php?accion=borrarRegistro&Id_del=${id_del}`;
...
function nextBotonCat() { //Botón calendario siguiente
  ...
  if (tiempo_seleccionado === "Anual") {
    if (parseInt(span_fecha.innerText) < 2023) {
      añoActual = parseInt(span_fecha.innerText);
      añoSiguiente = añoActual + 1;
...
function datosGraficaCatAnual(){
  ...
  let urlAn = `../back/php.php?yearCatAn=${año}`;
...
function datosGraficaCatMensual() {
  ...
  let urlMes = `../back/php.php?yearCatMes=${añoCatMes}&mesCatMes=${mesCatMes}`;
...
function añosFormInforme() {
  ...
  let url = `../back/php.php?añosInf=yes`;
...
function obtenerDatosInforme() {
  ...
  if (periodo_temp === "Anual") { //Petición y rellenado del informe Anual
    ...
    let urlAnual = `../back/php.php?añoInformeAn=${añoInf}`;

    fetch('./assets/plantillaInformeAnual.html', { //Primero: inserto la plantilla vacía en el HTML
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    let urlMes = `../back/php.php?añoInformeMes=${añoInf}&mesInforme=${mesInf}`;

    fetch('./assets/plantillaInformeMensual.html', { //Primero: inserto la plantilla vacía en el HTML
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
...
//Genera el PDF a partir de la imagen canvas de mi plantilla de informe rellena
